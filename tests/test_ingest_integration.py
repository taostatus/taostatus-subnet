...
def _bt_forecast(**overrides) -> dict:
    forecast = {
        "source": "bt_forecast",
        "uid": 0,
        "hotkey": "hotkey-0",
        "forecast_id": "fc-1",
        "run_id": "bt-2026-08-12",
...
def test_build_activity_payload_maps_known_event_type():
    ...
    payload = validator._build_activity_payload(_bt_forecast())

    assert payload["eventType"] == "significant_bittensor_event"
    assert payload["runId"] == "bt-2026-08-12"
    assert payload["questionKey"] == "q-key-1"
    assert payload["forecastId"] == "fc-1"
    assert payload["forecastWindow"] == "7d"
...
def test_build_activity_payload_falls_back_to_other_for_unknown_event_type():
    ...
    payload = validator._build_activity_payload(_bt_forecast(event_type="not_a_real_type"))
...
def test_build_activity_payload_marks_no_answer():
    validator = _validator(_FakeIngestClient())
    forecast = _bt_forecast(probability=None, prediction=None, confidence=None)
...
def test_queue_ingest_submission_is_noop_when_unconfigured():
    ...
    validator._queue_ingest_submission("fid-1", _bt_forecast())
...
def test_queue_ingest_resolution_skips_without_activity_id():
    ...
    validator._queue_ingest_resolution(_bt_forecast(), outcome=True, reward=0.8)
...
def test_queue_ingest_resolution_includes_brier_score():
    validator = _validator(_FakeIngestClient())
    forecast = _bt_forecast(ingest_activity_id="activity-1")
...
def test_flush_ingest_submissions_writes_activity_id_into_pending():
    ...
    validator.pending["fid-1"] = _bt_forecast()
    validator.ingest_submit_queue = [
...
def test_prune_masxai_state_bounds_ingest_queues(monkeypatch):
    ...
    validator.ingest_submit_queue = [{"fid": str(i)} for i in range(5)]
    validator.ingest_resolve_queue = [{"activity_id": str(i)} for i in range(5)]

    stats = validator.prune_masxai_state()

    assert len(validator.ingest_submit_queue) == 2
    assert len(validator.ingest_resolve_queue) == 2
    assert stats["ingest_submit"] == 3
    assert stats["ingest_resolve"] == 3
