...
# state and forecast activity, authenticated via a static X-API-Key (distinct
# from the BT-Forecast integration's bearer token). Fully opt-in: unless both
# env vars below are set, open_ingest_client_from_env() returns None and every
# validator hook that depends on it becomes a no-op.
