...
A separate backend from BT-Forecast: it accepts machine-to-machine reports of
miner metagraph state and forecast activity, authenticated with a static
X-API-Key header rather than a bearer token. Entirely optional - validators
that don't configure it never call out to it.
"""
